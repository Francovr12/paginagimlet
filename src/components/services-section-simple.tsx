...
// Componente simplificado para el efecto de escritura
...
export default function ServicesSection() {
  ...
  // Efecto para determinar si mostrar la animación
  useEffect(() => {
    // Verificar si ya se ha mostrado la animación antes
    const hasSeenAnimation = localStorage.getItem("hasSeenServicesAnimation") === "true"

    if (!hasSeenAnimation && inView) {
      setShowAnimation(true)

      // Marcar que la animación se ha mostrado después de un tiempo
      const timer = setTimeout(() => {
        localStorage.setItem("hasSeenServicesAnimation", "true")
      }, 5000) // Tiempo suficiente para que se complete la animación
  ...
  return (
    <section id="servicios" className="py-20 px-4 relative overflow-hidden">
          ...
          <div className="inline-block">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">
              ...
              <span className="text-purple-400 relative">
                Servicios
                <span className="absolute bottom-0 left-0 h-1 bg-purple-400 w-full" />
              </span>
            </h2>
          </div>
              ...
              <Card className="bg-black/40 border-purple-900/50 backdrop-blur-sm hover:bg-black/60 transition-all duration-300 h-full overflow-hidden">
                <CardHeader className="relative">
                  ...
                  <CardTitle className="text-xl text-white relative">
                    {service.title}
                    <div className="absolute -bottom-1 left-0 h-[2px] bg-purple-400/30 w-[30%]" />
                  </CardTitle>
                ...
                <CardContent>
                  <CardDescription className="text-purple-100/70 text-base min-h-[80px]">
                    {showAnimation ? <TypewriterEffect text={service.description} /> : service.description}
                  </CardDescription>
                ...
                <div className="absolute -top-10 -right-10 w-20 h-20 bg-purple-500/20 rounded-full blur-xl opacity-[0.2]" />

                {/* Efecto de línea que se dibuja */}
                <div className="absolute bottom-0 left-0 h-[2px] bg-gradient-to-r from-purple-500 to-transparent w-0 hover:w-full transition-all duration-800" />
              </Card>
            </div>
          ))}
        </div>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl h-full max-h-4xl rounded-full bg-purple-600/10 blur-[120px] -z-10" />

      {/* Efecto de escritura para el título de la sección - ajustado para móviles */}
      <div className="absolute top-10 left-1/2 transform -translate-x-1/2 text-purple-300/20 text-5xl md:text-8xl font-bold opacity-10 pointer-events-none select-none whitespace-nowrap">
        SERVICIOS
      </div>
    </section>
  )
